...
#for plotting
newdata.avsa_CFR <- newdata_CFR %>% mutate(species = "Avsa")
newdata.trae_CFR <- newdata_CFR %>% mutate(species = "Trae")
newdata.hovu_CFR <- newdata_CFR %>% mutate(species = "Hovu")
newdata.pegl_CFR <- newdata_CFR %>% mutate(species = "Pegl")
newdata.chga_CFR <- newdata_CFR %>% mutate(species = "Chga")
newdata.zema_CFR <- newdata_CFR %>% mutate(species = "Zema")
...
scale_fill_manual(values = mycols)+
 ...
 # geom_line(data=newdata.zema, aes(x=F.RWC, y=p.zema, color=species), size=2)+
   geom_vline(xintercept=ED50.avsa_CFR,color=mycols["Avsa"],lty=5, linewidth = 1)+
   geom_vline(xintercept=ED50.trae_CFR,color=mycols["Trae"],lty=5, linewidth = 1)+
   geom_vline(xintercept=ED50.hovu_CFR,color=mycols["Hovu"],lty=5, linewidth = 1)+
   geom_vline(xintercept=ED50.pegl_CFR,color=mycols["Pegl"],lty=5, linewidth = 1)+
   geom_vline(xintercept=ED50.chga_CFR,color=mycols["Chga"],lty=5, linewidth = 1)+
 #  geom_vline(xintercept=ED50.zema,color=mycols["Zema"],lty=5, linewidth = 1)+
  xlim(0,100)+
  theme_minimal()+
 theme(panel.grid.minor=element_blank(),
...
Among the remaining models, LN.4 also failed to provide stable estimates for Zema (very large SE), while LL.4 yielded consistent and biologically coherent parameter estimates across all species and had an AIC within ΔAIC < 2 of the minimum. We therefore selected LL.4 as the most appropriate and robust model.
```{r}
...
##Figure 3 - ALL PLOTS PLRC + CFD + CFR
...
CFR_without_legend <- CFR + theme(legend.position = "none")


Figure3 <- ggarrange(CFD_without_legend, CFR_without_legend, PLRC_without_legend, legend, ncol = 1, nrow = 4, labels = c("A", "B", "C", ""),heights = c(1,1,1,0.4) )
Figure3

ggsave(filename = "../results/Figure3.jpg", plot = Figure3,  width = 8, height = 12)
```
  ...
  mutate(
    across(
      ...
      .fns = list(psi = ~ {
        ...
        Ps <- m.osp.tlp / trait_value   # trait_value is 0–1
...
Kleafmax_Amax <-ggplot(Table_psi_rename , aes(x = Amax, y = Kleafmax, colour = Metabolism)) +
  ...
  annotate("text",
           ...
           parse= T,
           size = 5) +
  xlab(expression(italic(A)[nmax] ~ "(" * µmol ~ m^{-2} ~ s^{-1} * ")"))+
  ylab(expression(" "))+
...
```{r}
######
round(cor_res_s$r["πtlp", "PA50"], 2)
signif(cor_res_s$P["πtlp", "PA50"], 2)
round(cor_res_p$r["πtlp", "PA50"], 2)
signif(cor_res_p$P["πtlp", "PA50"], 2)
label_text <- "atop(italic(r)[s] ~ '=' * '0.89' ~ '*',italic(r)[p] ~ '=' * '0.95' ~ '**')"

tlp_A50 <-ggplot(Table_psi_rename, aes(y = πtlp , x = PA50, colour = Metabolism)) +
  geom_point(size = 3.6) +
  scale_color_manual(values = c(C3 = "#6DB9F8", C4 = "#D9AB33")) +
  geom_smooth(
    aes(y = πtlp , x = PA50),   # re-specify to ignore grouping
...
############
round(cor_res_s$r["πtlp", "Pgs50"], 2)
signif(cor_res_s$P["πtlp", "Pgs50"], 2)
round(cor_res_p$r["πtlp", "Pgs50"], 2)
signif(cor_res_p$P["πtlp", "Pgs50"], 2)
label_text <- "atop(italic(r)[s] ~ '=' * '0.94' ~ '**',italic(r)[p] ~ '=' * '0.86' ~ '*')"

tlp_gs50 <-ggplot(Table_psi_rename, aes(y = πtlp, x = Pgs50, colour = Metabolism)) +
  geom_point(size = 3.6) +
  scale_color_manual(values = c(C3 = "#6DB9F8", C4 = "#D9AB33")) +
  geom_smooth(
    aes(y = πtlp, x = Pgs50),   # re-specify to ignore grouping
    method = "lm", se = FALSE, color = "black", linetype = "dashed",
    inherit.aes = FALSE ) +
  annotate("text",
           x = -0.95,
...
####
round(cor_res_s$r["πtlp", "PKleaf50"], 2)
signif(cor_res_s$P["πtlp", "PKleaf50"], 2)
round(cor_res_p$r["πtlp", "PKleaf50"], 2)
signif(cor_res_p$P["πtlp", "PKleaf50"], 2)
label_text <-"atop(italic(r)[s] ~ '=' * '0.49' ~ 'ns',italic(r)[p] ~ '=' * '0.31' ~ 'ns')"

tlp_Kleaf50 <-ggplot(Table_psi_rename, aes(y = πtlp, x = PKleaf50, colour = Metabolism)) +
  geom_point(size = 3.6) +
  scale_color_manual(values = c(C3 = "#6DB9F8", C4 = "#D9AB33")) +
  geom_smooth(
    aes(y = πtlp, x = PKleaf50),   # re-specify to ignore grouping
    method = "lm", se = FALSE, color = "black", linetype = "dashed",
    inherit.aes = FALSE ) +
  annotate("text",
           x = -0.4,
...
####
round(cor_res_s$r["πtlp", "Embolism onset"], 2)
signif(cor_res_s$P["πtlp", "Embolism onset"], 2)
round(cor_res_p$r["πtlp", "Embolism onset"], 2)
signif(cor_res_p$P["πtlp", "Embolism onset"], 2)
label_text <-"atop(italic(r)[s] ~ '=' * '-0.94' ~ '**',italic(r)[p] ~ '=' * '-0.97' ~ '**')"

tlp_embolism <-ggplot(Table_psi_rename, aes(y = πtlp, x = `Embolism onset`, colour = Metabolism)) +
  geom_point(size = 3.6) +
  scale_color_manual(values = c(C3 = "#6DB9F8", C4 = "#D9AB33")) +
  geom_smooth(
    aes(y = πtlp, x = `Embolism onset`),   
    method = "lm", se = FALSE, color = "black", linetype = "dashed",
    inherit.aes = FALSE ) +
  annotate("text",
           y = -0.8,
