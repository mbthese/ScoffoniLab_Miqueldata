...
result_path <- "C:/Users/mboisse/Dropbox/Mon PC (Jaboty20)/Post_doc_CalState_LA/Miquel_data/results"
```
...
```{r}
mybootPX <- function(fit, px = 0.5, psi_max, seed = 123, sims = 1000){

  fit.list <- attr(fit, "fit.list")

  if(fit.list){ 
    n_fit <- length(fit) #if the input is a list of models, checks the number of models and extract each model's type
    model.test <- lapply(seq_len(n_fit), \(x) fit[[x]]$data.type) %in% c("exp2", "log", "sig", "Linear", "exp")
  }else{
    n_fit <- 1 #if the input is 1 model, check that one
...
output <- vector(mode = "list", length = n_fit) #prepares list to store result for each fit
...
for(i in seq_len(n_fit)){
  ...
  px_est <- switch(px_char, #If the user asked for P50 at 0 MPa, grab psi_k50. If they asked for P50 at 0.1 MPa, grab psi_k50_at0.1. Same idea for P80.”
                   "0.5@"=fit_temp$psi_k50,
                   "0.8@"=fit_temp$psi_k80,
                   "0.5@0.1"=fit_temp$psi_k50_at0.1,
                   "0.8@0.1"=fit_temp$psi_k80_at0.1) #Loops over fits and pulls the “observed” (non-bootstrapped) PX estimate.

  fit_resample <- resamplePX(fit = fit_temp, sims = sims,
                        px = px, psi_max = psi_max, seed = seed) #the actual bootstrap

  finite_values <- sapply(fit_resample, function(x) is.finite(x[[1]])) #Filters out non-finite values (NaN, Inf).
  ...
  t_score = qt(p=alpha/2, df=deg_of_freedom, lower.tail = F) #Uses Student’s t distribution to calculate margin of error
  margin_error <- t_score*boot_se

 conf.low <- px_est-margin_error # using the predicted pX value to make the error make sense, Builds confidence interval around the model-estimated px_est (not the bootstrap mean!)
...
result_path <- "C:/Users/mboisse/Dropbox/Mon PC (Jaboty20)/Post_doc_CalState_LA/Miquel_data/results/KplantKroots_results"
...
p_trait <- boottable_final%>%
  ggplot(aes(x=Species_name, y=px_est, ymin=conf.low, ymax=conf.high, color=type)) +
...
# Plot for trait A
p_A <- ggplot(boottable_final[boottable_final$trait == "A", ],
              aes(x = Species_name, y = px_est,
                  ymin = conf.low, ymax = conf.high, color = type)) +
  geom_pointrange(position = position_dodge(width = 0.5), size = 0.8) +
  coord_flip() +
  labs(y = "P50 (-MPa)", x = "",
       color = "Metabolism",
