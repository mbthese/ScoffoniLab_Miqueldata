...
define_parsS <- function(input_df) {
  # --- General case ---
    parsS <- list(A = max(input_df$gs), B = -0.5, Xo = mean(range(input_df$psi, na.rm = TRUE)), sd = 2)
    par_loS <- list(A = 0, B = -1.25, Xo = min(input_df$psi, na.rm = TRUE) - 0.1, sd = 0.0005)
    par_highS <- list(A = max(input_df$gs) * 1.5, B = 0, Xo = max(input_df$psi, na.rm = TRUE) + 0.1, sd = 50)
...
# === MAIN EXECUTION ===
input_file <- "../results/A/A.csv"
output_dir <- "../results/A/A_anneal_modified/"
...
for (sp in species_list) {
  for (dt in datatype_list) {
    ...
    if (nrow(subset_df) < 3) next   # Skip if too few points
    
    # --- Exponential ---
    pE <- define_parsE(subset_df)
    fitE <- do_the_thing_nonlinear(subset_df, Exponential, pE$parsE, pE$par_loE, pE$par_highE, "Exponential", plot_dir)
    ...
    fitE2 <- do_the_thing_nonlinear(subset_df, Exponential2, pE2$parsE2, pE2$par_loE2, pE2$par_highE2, "Exponential2", plot_dir)
    ...
    fitL <- do_the_thing_nonlinear(subset_df, Logistic, pL$parsL, pL$par_loL, pL$par_highL, "Logistic", plot_dir)
    all_results_r[[paste(sp, dt, "Logistic", sep="_")]] <- fitL
    all_results_df <- rbind(all_results_df, fitL$summary)
    
    # --- Sigmoidal ---
    pS <- define_parsS(subset_df)
    fitS <- do_the_thing_nonlinear(subset_df, Sigmoidal, pS$parsS, pS$par_loS, pS$par_highS, "Sigmoidal", plot_dir)
...
# === SAVE RESULTS ===
write.csv(all_results_df, file.path(output_dir, "model_fitting_results_A_anneal_modified.csv"), row.names = FALSE)
saveRDS(all_results_r, file.path(output_dir, "model_fitting_results_full.RDS"))
